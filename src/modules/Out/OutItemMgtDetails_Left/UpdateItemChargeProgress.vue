				...
				<div class="progress" ref="progress" oncopy="function(){alert(1)}">
					...
					<div ref="circle" class="circle">
						<div class="show" ref="show">
							...
							<div class="shownum">
								<div class="o"></div>
								<span>{{selfParseFloat(proform.current_num)}}%</span>
							</div>
						</div>
					</div>
					<!--上一次的更新进度-->
...
<script>
	...
	export default {
		...
		computed: {
			busIdComp() {
				if (this.$store.getters.outItemItem.busId) {
					return this.$store.getters.outItemItem.busId
				} else {
		...
		methods: {
			...
			sure() {
				let f = {
					...
					progress: this.proform.current_num,
					busId: this.busIdComp
				}
				this.$http.post('/api/ioRecord/saveItemProgressByItemOutCharge', f, {
						emulateJSON: false
					})
					.then((res) => {
			...
			obtainRemoteDataSource(callback) {
				this.$http.post('/api/ioRecord/detailItemProgressByItemOutByCharge', {
						applyId: +this.$route.params.id.split('-')[0],
						busId: this.busIdComp
					}, {
						emulateJSON: true
					...
					.then((res) => {
						...
						if(res.data.success) {
							...
							for(let i in ipRecordList) {
								this.proform.lastnum.push(Object.isEmpty(ipRecordList[i].progress) ? 0 : ipRecordList[i].progress)
							}
							this.proform.current_num = this.proform.lastnum[this.proform.lastnum.length - 1] || 0
							callback()
						}
					}).catch((err) => {
