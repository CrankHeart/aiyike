		...
		<div class="project_operation_btn" @click.stop="menuClick" v-if="popshow">
			...
			<transition name="fade">
				<ul class="menu" v-show="show" ref="menu">
					<li ref="items" @click.stop="cancelStateClick" v-if="reHangUpVisible && HasPermission('giveUpHangUp')">解除挂起</li>
					<li ref="items" @click.stop="outItemStateDialogClick('hangUpByOut')" v-if="btnVisible && HasPermission('hangUpByOut')">项目挂起</li>
					<li ref="items" @click.stop="outItemStateDialogClick('stopByOut')" v-if="btnVisible && HasPermission('stopByOut')">项目中止</li>
					<li ref="items" @click.stop="outItemStateDialogClick('finishByOut')" v-if="btnVisible && HasPermission('finishByOut')">项目结项</li>
					<li ref="items" @click.stop="peripheralTeamDialogClick" v-if="HasPermission('peripheralTeam') && btnVisible">评价外围团队</li>
					<li ref="items" @click.stop="updateItemProgressClick" v-if="HasPermission('updateOutProgress') && btnVisible">更新项目进度</li>
					<li ref="items" @click.stop="itemReportClick" v-if="HasPermission('phasedReporting') && btnVisible">阶段性汇报</li>
					<li ref="items" @click.stop="assessAiyiChargePersonDialogClick" v-if="HasPermission('assessAiyiCharge') && btnVisible">评价艾艺负责人</li>
					<li ref="items" @click.stop="updateItemChargeProgressClick" v-if="HasPermission('updateItemChargeProgress') && btnVisible">外围更新进度</li>
				</ul>
			</transition>
		</div>
		<component :is="outItemStateDialogView" @refreshList="onRefresList" :titleType="outItemStateType" :visible.sync="outItemStateDialogFlag"></component>
...
<script>
//项目挂起 项目中止 项目结项
let OutItemStateDialog = () => import("./OutItemStateDialog.vue");
//评价外围团队
let PeripheralTeamDialog = () => import("./PeripheralTeamDialog.vue");
//更新项目进度
let UpdateItemProgress = () => import("./UpdateItemProgress.vue");
//评价艾艺负责人
let AssessAiyiChargePersonDialog = () =>
  import("./AssessAiyiChargePersonDialog.vue");
let UpdateItemChargeProgress = () => import("./UpdateItemChargeProgress.vue");
...
export default {
  ...
  computed: {
    ...
    hanupBtnVsibel() {
      if (this.state == 11 || this.state == 12 || this.state == 13) {
        return false;
      } else {
        return true;
      }
  ...
  methods: {
    ...
    itemReportClick() {
      this.$router.push(
        `/out/outMgt/reporting/${this.$route.params.id.split("-")[0]}`
      );
    },
    menuClick() {
      this.show = !this.show;
...
.project_operation_btn {
  ...
  bottom: 24px;
  background: #295dcf;
  box-shadow: 0 5px 4px 0 rgba(41, 93, 207, 0.1);
  width: 50px;
  height: 50px;
  border-radius: 100%;
  color: #fff;
