		...
		<section class="layout-list">
			...
			<el-form :inline="true" :model="formInline" class="soso clearfix margin-left8" label-width="68px">
				<!--<el-form-item class="label inlineblock" label="提交日期：">-->
					<!--<el-date-picker class="input_text" type="date" placeholder="开始日期" v-model="formInline.starttime" style="width: 115px"></el-date-picker>-->
					<!--<span>-</span>-->
					<!--<el-date-picker class="input_text" type="date" placeholder="结束日期" v-model="formInline.endtime" style="width: 115px"></el-date-picker>-->
				<!--</el-form-item>-->
				...
				<el-form-item class="label marginLeft18 form190" label="周报内容：">
					<!--<el-select v-model="weeklyTypeValue" class="label select-wrap" placeholder="请输入关键字" v-sicon1 @change="showPName">-->
						<!--<el-option v-for="item in weeklyType" :key="item.value" :label="item.label" :value="item.value">-->
						<!--</el-option>-->
					<!--</el-select>-->
					<el-input v-model="formInline.weeklycontent" class="select-wrap" filterable placeholder="请输入关键字"></el-input>
				</el-form-item>
				<el-form-item class="label block" label="项目名称：" v-if="showProjectName">
					<el-select v-model="entryNameValue" class="select-wrap" filterable multiple placeholder="选择项目名称" v-sicon1>
						<el-option v-for="item in entryName" :key="item.id" :label="stringSplit(item.name)" :value="item.id">
						</el-option>
					</el-select>
				</el-form-item>
				<el-form-item class="label btngroup">
			...
			<div class="log-list clearfix" ref="panelWrap">
				<template v-for="(item,key) in weeklyList">
					<ProjectWeekly v-if="item.category == 1" :key="key" :item="item" ref="currWeeklyList" class="fl currWeeklyList ProjectWeekly" @hover="showOperationOrNot" @click.native="ProjectWeeklyDeatils(key,item)"></ProjectWeekly>
					<BusinessWeekly v-if="item.category == 2" :key="key" :item="item" ref="currWeeklyList" :ishoverOrNot="item.status == 0||item.status == 2 ? true:false" class="fl currWeeklyList BusinessWeekly" @hover="showOperationOrNot" @click.native="departmentWeeklyDetails(key,item)">
	...
	export default {
		...
		methods: {
			...
			dealSosoData() {
				...
				this.data.type = !!(this.weeklyTypeValue) ? this.weeklyTypeValue : ''
            ...
            forGetWeeklyDom(){
                ...
                let timer = requestAnimationFrame(function fn(){
                    if (_self.$refs.currWeeklyList && _self.$refs.currWeeklyList.length == _self.weeklyList.length) {				//定时器执行
                        cancelAnimationFrame(timer)				//清除定时器
                        let goingTime = new Date().getTime()
						if(goingTime-currTime>=5000){
							return
						}
						//执行相应的逻辑
            ...
            resetMargins(mval,num){
                var lis = document.getElementsByClassName("WeeklyManagement_My")[0].querySelectorAll(".currWeeklyList")
