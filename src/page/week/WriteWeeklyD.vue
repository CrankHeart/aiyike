				...
				<section class="layout">
					<div class="writeform-wrap">
						...
						<el-form :inline="true" :model="formInline" ref="formInline" class="demo-form-inline" label-position="left" label-width="75px">
							<el-form-item label="部门名称：" class="form190">
								<el-input :disabled="true" v-model="formInline.bu_name" placeholder="部门名称"></el-input>
							</el-form-item>
	...
	export default {
		...
		methods: {
			...
			getBasicInfor() {
				...
				this.formInline.bu_name = this.dataList.depart.label || ""
			},
			loadUeJs() {
				let configPomise = null,
					allPomise = null,
					zhPomise = null,
					parsePomise = null,
					_self = this
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if(valid) {
						...
						if(this.editStatus) {
							subData.logId = this.editData.id
							this.$http.post('/api/week/modify', subData, {
									emulateJSON: false
								})
								.then((res) => {
									if(res.data.success) {
							...
							this.$http.post('/api/week/add', subData, {
									emulateJSON: false
								})
								.then((res) => {
									var data = res.data;
