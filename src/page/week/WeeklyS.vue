...
<script>
    ...
    let WeeklyList = function(resolve) {
        require(['@/modules/week/WeeklyListS'], resolve);
    }
    let WeeklyContentProjectS = function(resolve) {
        require(['@/modules/week/WeeklyContent_ProjectS'], resolve);
    }
    let WeeklyDetailsComponentsD = function(resolve) {
    ...
    export default {
        ...
        watch:{
            "state.WeeklyDetalisData":{
                handler(val,oldVal){
                    if(val.category == 1){
                        this.showRightContent = WeeklyContentProjectS           //项目类型周报右侧
                        this.rightWeeklyList = val
                    }else if(val.category == 2){
                        this.showRightContent = WeeklyDetailsComponentsD            //部门周报右侧
                        this.rightWeeklyList = val
        ...
        methods: {
            judgeRightContent(){
                ...
                if(val.category == 1){
                    this.showRightContent = WeeklyContentProjectS
                    this.rightWeeklyList = val
                }else if(val.category == 2){
            ...
            goBackToContent(){
                switch (routeFrom.path){
                    ...
                    case '/weeklymanagement/my':
                        ...
                        break
                    case '/weeklymanagement/search':
                        this.$router.push('/weeklymanagement/search')
                        break
                    case '/weeklymanagement/osearch':
                        this.$router.push('/weeklymanagement/osearch')
                        break;
                    default:
                        this.$router.push('/weeklymanagement')
                }
