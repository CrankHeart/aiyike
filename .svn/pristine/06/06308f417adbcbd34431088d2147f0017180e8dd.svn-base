<template>
	<section class="wrap-o">
		...
		<el-form :model="searchParam" ref="searchParam"  class="soso clearfix margin-left8" label-width="70px">
			<el-form-item class="label inlineblock startend" label="起止日期：">
			...
			<el-form-item class="label inlineblock namecontent name_label" label="姓名：" prop="ucId">
				<el-select v-model="searchParam.ucId" filterable clearable  placeholder="请选择">
					<el-option v-for="item in ucNames" :key="item.id" :label="item.name" :value="item.id">
					</el-option>
				</el-select>
			</el-form-item>
			<el-form-item class="label inlineblock namecontent marginleft9" label="所属项目：" prop="itemId">
			...
			<el-form-item class="label inlineblock namecontent" label="所属部门：" prop="departId">
			...
			<el-form-item class="label namecontent inlineblock marginleft9" label="日志内容：" prop="content">
		...
		<LogDetails :visible.sync="logDetailsVisible":reportLogList="reportLogData" :dayLog="dayLog" :logComments="logComments"></LogDetails>
		</div>
	</section>
</template>
...
<script>
	...
	let LogDetails = () => import('./LogDetails.vue')
     ...
     /*       this.nameList = this.nameStates.map(item => {
        ...
        methods: {
        	mouseup(e) {
				...
				if(/input|   /i.test(target.tagName) && /firefox/i.test(navigator.userAgent)) {
					//Firefox在文本框内选择文字
            	...
            	if (this.textSelectFlag) {
	                this.$http.post('/api/reportLog/queryReportLogAndDayLog?id='+row , {
	                	...
	                	if(res.data.success){
							 ...
							 this.logComments=result.logComments
            ...
            onSubmit(curPage) {
                ...
                this.$http.post('/api/dayLog/queryDayLogListByPm',this.searchParam,{
                    emulateJSON: true
                }).then((res) => {
            ...
            queryItems() {
                this.$http.post('/api/dayLogQuery/queryItemsByPm',this.searchParam,{
                    emulateJSON: true
                }).then((res) => {
            ...
            queryNames() {
                this.$http.post('/api/dayLogQuery/queryNamesByPm',this.searchParam,{
                    emulateJSON: true
                }).then((res) => {
            ...
            queryDeparts() {
                ...
                this.$http.post('/api/dayLogQuery/queryDepartsByPm',this.searchParam,{
                    emulateJSON: true
                }).then((res) => {
